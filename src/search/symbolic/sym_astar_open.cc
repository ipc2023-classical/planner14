...
void SymAstarOpen::extract_states(Bucket & bucket, int f, int g,
    ...
    if (open) exp->filterDuplicates (bucket);
...
int SymAstarOpen::getNextG(int f, int g) const {
	...
	if(nextGBucket->first > f || exp->rejectLargerG(f, nextGBucket->first)) break;
	if(exp->acceptFG(f, nextGBucket->first)) return nextGBucket->first;
    }
